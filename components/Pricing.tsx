...
export function Pricing() {
  ...
  return (
    <section id="pricing" className="section-padding relative overflow-hidden bg-ink-950 text-white">
      ...
      <div className="relative mx-auto max-w-6xl px-5">
        ...
        <Reveal className="mt-10 flex flex-col items-center gap-2">
          ...
          <motion.span
            ...
            className="rounded-full bg-amber-500/15 px-3 py-1 text-[11px] font-semibold text-amber-400"
          >
            Save 42% with annual
          </motion.span>
        </Reveal>
        ...
        <div className="mx-auto mt-10 grid grid-cols-1 max-w-4xl gap-6 md:grid-cols-2">
            ...
            <div className="flex h-full flex-col rounded-card-lg border border-white/[0.08] bg-white/[0.03] p-8 backdrop-blur-sm">
              ...
              <ul className="mt-6 space-y-3">
                {free.features.map((f) => (
                  <li
                    key={f}
                    className="flex items-start gap-2 text-base text-ink-200"
                  >
                    <Icon.check
                      width={17}
          ...
          <Reveal delay={0.1}>
            <div className="gradient-border relative flex h-full flex-col rounded-card-lg bg-gradient-to-b from-white/[0.08] to-white/[0.03] p-8 shadow-[0_0_80px_rgba(245,158,11,0.1)] backdrop-blur-sm ring-1 ring-amber-500/20">
              {/* Trial badge */}
              <div className="absolute right-5 top-5 z-10 rounded-full bg-amber-500 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-ink-950">
                7-day free trial
              </div>
              ...
              <ul className="mt-6 space-y-3">
                {pro.features.map((f) => (
                  <li
                    key={f}
                    className="flex items-start gap-2 text-base text-ink-100"
                  >
                    <Icon.check
                      width={17}
